...
// generateMetadata() rather than a static export, so seo.title/seo.description
// (site_content) drive the title/description instead of the original
// hardcoded copy — see project-admin-panel memory.
export async function generateMetadata(): Promise<Metadata> {
  const content = await getSiteContent();
  const title = content["seo.title"] || "Synchro Media — Automotive Photography, Atlanta";
  const description =
  ...
  return {
    ...
    title: { default: title, template: "%s — Synchro Media" },
    description,
    openGraph: {
      type: "website",
